...
Hermes Agent uses the same hosted MCP bridge, but its official MCP surface is configured in:

```text
~/.hermes/config.yaml
```

with a `mcp_servers.velocitybrain` entry that points to `velocitybrain serve mcp`.
