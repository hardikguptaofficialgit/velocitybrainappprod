...
export default function VelAiOnboardingChat({ form, onFormChange, onComplete, onFallbackToManual }) {
  ...
  const serverCompleteRef = useRef(false);
  ...
  const appendAssistant = useCallback((text, widget = null) => {
    ...
    setMessages((current) => {
      const lastAssistant = [...current].reverse().find((entry) => entry.role === 'assistant');
      if (lastAssistant && normalizeChatText(lastAssistant.text) === normalized) {
        return current;
      }
      return [
        ...current,
  ...
  const isChatComplete = useCallback(
    () => serverCompleteRef.current || isVelAiInfoComplete(formRef.current, serverCompleteRef.current),
    []
  );

  const runAssistantTurn = useCallback(async ({ userMessage = '', bootstrap = false, history } = {}) => {
    if (!bootstrap && isChatComplete()) {
      return;
    }
    ...
    try {
      const messageHistory = history ?? (bootstrap ? [] : messages);
      const result = await callVelAi({
        userMessage,
        bootstrap,
        history: bootstrap ? [] : messageHistory
      });

      applyServerResult(result, formRef.current);
      if (result.complete) {
        serverCompleteRef.current = true;
      }
  ...
  }, [appendAssistant, applyServerResult, callVelAi, isChatComplete, messages]);
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="mb-2 flex items-center justify-between gap-2 text-[10px] text-zinc-600">
        <span>
          {aiReady === null ? 'Connecting…' : aiReady ? 'Powered by VelAI' : 'Basic mode — set GITHUB_TOKEN on backend'}
        </span>
      </div>

      <div
        ref={scrollRef}
        className="min-h-0 flex-1 space-y-3 overflow-y-auto rounded-lg  bg-[#0c0c0c] px-3 py-3"
      >
      ...
      {quickReplies.length > 0 && !complete && (
        <div className="mt-2 flex flex-wrap gap-1.5">
          {quickReplies.map((reply) => (
            <button
              key={reply}
              type="button"
              disabled={typing || complete}
      ...
      <form onSubmit={handleSend} className="mt-3 flex-shrink-0">
        ...
        <div className="flex items-end gap-2">
          <textarea
            ...
            rows={1}
            placeholder={complete ? 'Setup complete' : 'Message VelAI…'}
            disabled={typing || complete}
            className="min-h-[40px] max-h-20 flex-1 resize-none rounded-lg  bg-[#0c0c0c] px-3 py-2 text-[13px] text-white outline-none placeholder:text-zinc-600 focus:border-zinc-600 disabled:opacity-50"
          />
          <button
            type="submit"
          ...
          >
            {typing ? <BlobLoader size={18} label="" variant="accent" /> : 'Send'}
          </button>
        </div>
        <p className="mt-1.5 text-center text-[10px] text-zinc-600">
          {complete ? 'Profile complete — press Continue below to pick your avatar' : 'Enter to send'}
        </p>
      </form>
    </div>
  );
