...
const Dashboard = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto w-full space-y-8 pb-12">
      ...
      {isError && (
        <div className="rounded-xl border border-red-900/40 bg-red-900/10 p-4 text-red-300 text-sm shadow-inner">
      ...
      <div className="relative rounded-xl p-[1px] bg-gradient-to-b from-zinc-800/50 to-zinc-900/10">
        <div className="rounded-xl bg-[#0d0d0d] p-5 h-full">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <Database className="h-4 w-4 text-[#EA803A]" />
                <h3 className="text-base font-semibold bg-clip-text text-transparent bg-gradient-to-r from-white to-zinc-400" style={{ fontFamily: 'Syne, sans-serif' }}>Company Source Coverage</h3>
              </div>
              <p className="mt-1 text-sm text-zinc-400">{sourceCoverage.sourceCoverageSummary}</p>
            </div>
            <Link
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {connectedSources.length > 0 ? connectedSources.map((source) => (
              <span key={source.provider} className="rounded-lg border border-zinc-700 bg-zinc-800/40 px-3 py-1.5 text-xs text-zinc-300 shadow-inner">
                {source.label || source.provider} <span className="mx-1 text-zinc-600">•</span> <span className="text-zinc-500">{source.lastSyncStatus || 'idle'}</span>
              </span>
            )) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 relative rounded-xl p-[1px] bg-gradient-to-b from-zinc-800/50 to-zinc-900/10">
        ...
        <div className="relative rounded-xl p-[1px] bg-gradient-to-b from-zinc-800/50 to-zinc-900/10">
          <div className="bg-[#0d0d0d] rounded-xl p-6 flex flex-col h-full">
            <h3 className="text-base font-semibold bg-clip-text text-transparent bg-gradient-to-r from-white to-zinc-400 mb-2" style={{ fontFamily: 'Syne, sans-serif' }}>Operation Mix</h3>
            ...
            <div className="mt-4 space-y-2">
              {usageByEndpoint.length > 0 ? usageByEndpoint.map((item, index) => (
                <div key={item.endpoint} className="flex items-center justify-between text-sm p-2 rounded-lg bg-zinc-900/40 border border-zinc-800/50 shadow-inner">
        ...
        <div className="relative rounded-xl p-[1px] bg-gradient-to-b from-zinc-800/50 to-zinc-900/10">
          <div className="bg-[#0d0d0d] rounded-xl overflow-hidden flex flex-col h-full">
            <div className="p-5 border-b border-[#1c1c1c] flex items-center justify-between">
              <h3 className="text-base font-semibold bg-clip-text text-transparent bg-gradient-to-r from-white to-zinc-400" style={{ fontFamily: 'Syne, sans-serif' }}>Connected Agents</h3>
              <Link to="/dashboard/agents" className="text-xs text-[#EA803A] hover:text-[#f0965a] transition-colors flex items-center gap-1 font-medium">
                View All <ArrowRight className="w-3 h-3" />
              </Link>
            </div>
            <div className="divide-y divide-[#1c1c1c] overflow-y-auto">
              {(agentBreakdown.length > 0 ? agentBreakdown : supportedAgents.slice(0, 4).map((agent) => ({ agentId: agent.id, calls: 0, costUsd: 0, totalTokens: 0 }))).slice(0, 4).map((item) => {
                ...
                return (
                  <div key={agent.id} className="p-4 hover:bg-zinc-900/40 transition-colors bg-transparent">
                    <div className="flex items-center gap-3">
                      <AgentBrandIcon agentId={agent.id} name={agent.name} containerClassName="w-9 h-9 shrink-0 border border-current/10 shadow-inner" size="h-5 w-5" />
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <h4 className="text-sm font-medium text-white">{agent.name}</h4>
                          <span className="px-1.5 py-0.5 rounded border border-blue-500/20 bg-blue-500/10 text-[10px] text-blue-400 uppercase tracking-wide">{agent.surface}</span>
                        </div>
        ...
        <div className="relative rounded-xl p-[1px] bg-gradient-to-b from-zinc-800/50 to-zinc-900/10">
          <div className="bg-[#0d0d0d] rounded-xl overflow-hidden flex flex-col h-full">
            <div className="p-5 border-b border-[#1c1c1c]">
              <h3 className="text-base font-semibold bg-clip-text text-transparent bg-gradient-to-r from-white to-zinc-400" style={{ fontFamily: 'Syne, sans-serif' }}>Repository Activity</h3>
            </div>
            <div className="flex-1 overflow-auto max-h-[320px]">
              {topReusableRepos.length > 0 ? (
                <div className="divide-y divide-[#1c1c1c]">
                  {topReusableRepos.map((repo) => (
                    <div key={`${repo.repoId}-${repo.branch || 'main'}`} className="p-4 flex items-center justify-between hover:bg-zinc-900/40 transition-colors gap-4 bg-transparent">
                      <div className="min-w-0 pr-4">
                        <p className="text-sm font-medium text-zinc-200 truncate">{repo.repoName || repo.repoId}</p>
                        <p className="text-[10px] text-zinc-500 truncate mt-0.5">
                          {(repo.branch || 'default')} <span className="mx-1">•</span> {(repo.agents || []).join(', ') || 'workspace'}
                        </p>
                      </div>
                      <div className="text-right shrink-0">
                        <p className="text-sm font-bold text-[#EA803A]">{repo.calls || 0}</p>
                        <p className="text-[10px] text-zinc-500 mt-0.5">{Number(repo.saved || 0).toLocaleString()} tokens saved</p>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div className="xl:col-span-2 relative rounded-xl p-[1px] bg-gradient-to-b from-zinc-800/50 to-zinc-900/10">
          <div className="bg-[#0d0d0d] rounded-xl overflow-hidden flex flex-col h-full">
            <div className="p-5 border-b border-[#1c1c1c]">
              <h3 className="text-base font-semibold bg-clip-text text-transparent bg-gradient-to-r from-white to-zinc-400" style={{ fontFamily: 'Syne, sans-serif' }}>Recent Agent Activity</h3>
            </div>
            <div className="flex-1 overflow-auto max-h-[320px]">
              {recentActivity.length > 0 ? (
                <div className="divide-y divide-[#1c1c1c]">
                  {recentActivity.map((activity, index) => (
                    <div key={`${activity.timestamp}-${index}`} className="p-4 hover:bg-zinc-900/40 transition-colors bg-transparent">
                      <div className="flex items-center justify-between mb-1.5 gap-3">
                        <p className="text-sm font-medium text-white truncate pr-4">{activity.description}</p>
                        <span className={`shrink-0 px-2 py-0.5 rounded border text-[10px] font-medium uppercase tracking-wide ${activity.status === 'completed' ? 'bg-[#5fd1b3]/10 text-[#5fd1b3] border-[#5fd1b3]/20' : 'bg-red-500/10 text-red-400 border-red-500/20'}`}>
                          {activity.status}
                        </span>
                      </div>
        ...
        <div className="flex flex-col gap-4">
          <div className="relative rounded-xl p-[1px] bg-gradient-to-b from-zinc-800/50 to-zinc-900/10">
            <div className="bg-[#0d0d0d] rounded-xl p-5 h-full">
              <h3 className="text-base font-semibold bg-clip-text text-transparent bg-gradient-to-r from-white to-zinc-400 mb-4" style={{ fontFamily: 'Syne, sans-serif' }}>Actionable Signals</h3>
              <div className="space-y-3">
                {(anomalies.length > 0 ? anomalies.slice(0, 2) : insights.slice(0, 2)).map((item, index) => (
                  <div key={`${item.type || item.title}-${index}`} className={`rounded-lg border p-3 shadow-inner ${item.message ? 'border-red-500/30 bg-red-500/5' : 'border-[#202020] bg-zinc-900/50'}`}>
                ...
                {anomalies.length === 0 && insights.length === 0 && (
                  <p className="text-sm text-zinc-500 rounded-lg border border-dashed border-[#2a2a2a] bg-zinc-900/30 p-3 shadow-inner">
                    Signals will appear here as paired agents start producing telemetry.
                  </p>
                )}
              </div>
            </div>
          ...
          <div className="relative rounded-xl p-[1px] bg-gradient-to-b from-zinc-800/50 to-zinc-900/10">
            <div className="bg-[#0d0d0d] rounded-xl p-5 h-full">
              <h3 className="text-sm font-semibold bg-clip-text text-transparent bg-gradient-to-r from-white to-zinc-400 mb-4">Model Breakdown</h3>
              <div className="space-y-3">
                {(modelBreakdown.length > 0 ? modelBreakdown : [{ modelName: 'No model data', calls: 0, costUsd: 0 }]).slice(0, 4).map((model, idx) => (
                  <div key={idx} className="flex items-center justify-between text-sm p-2 rounded-lg bg-zinc-900/40 border border-zinc-800/50 shadow-inner">
                    <div className="min-w-0 pr-3">
                      <p className="text-white truncate font-medium">{model.modelName}</p>
          ...
          <div className="relative rounded-xl p-[1px] bg-gradient-to-b from-zinc-800/50 to-zinc-900/10 flex-1">
            <div className="bg-[#0d0d0d] rounded-xl p-5 h-full">
              <h3 className="text-sm font-semibold bg-clip-text text-transparent bg-gradient-to-r from-white to-zinc-400 mb-4">Memory Lifecycle</h3>
              <div className="space-y-4 relative before:absolute before:inset-0 before:ml-2.5 before:-translate-x-px before:h-full before:w-0.5 before:bg-gradient-to-b before:from-[#EA803A] before:to-transparent pt-2">
                {promptLifecycle.slice(0, 3).map((item, idx) => (
                  <div key={idx} className="relative flex items-center gap-3">
                    <div className="w-5 h-5 rounded-full bg-[#161616] border-2 border-[#EA803A] shadow-md shadow-[#EA803A]/20 flex items-center justify-center shrink-0 text-[10px] font-bold text-[#EA803A] z-10">
                      {idx + 1}
                    </div>
                    <p className="text-xs text-zinc-400 leading-tight bg-zinc-900/40 border border-zinc-800/50 p-2 rounded-lg shadow-inner flex-1">
                      <span className="text-zinc-200 font-medium block mb-0.5">{item.title}</span>
